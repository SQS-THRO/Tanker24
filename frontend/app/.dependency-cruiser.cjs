...
module.exports = {
	forbidden: [
		...
		},
		{
			...
			to: {
				...
				dependencyTypesNot: ['type-only'],
				pathNot: ['node_modules/@types/', 'node_modules/@playwright/', 'node_modules/svelte/']
			}
		},
		{
		...
		{
			name: 'routes-not-to-import-routes',
			severity: 'error',
			comment: 'Routes (pages) should not import from other routes to prevent coupling between pages.',
			from: { path: '^src/routes/', pathNot: ['[.]e2e[.]'] },
			to: { path: '^src/routes/' }
		},
		{
			name: 'components-not-to-import-routes',
			severity: 'error',
			comment: 'Reusable UI components should not depend on page layout or routing.',
		...
		},
		{
			...
			severity: 'error',
			comment: 'Components should not directly call API services. Use a store as an intermediary.',
			from: { path: '^src/lib/components/' },
			to: { path: '^src/lib/services/' }
		},
		{
			name: 'services-not-to-internals',
			severity: 'error',
			comment: 'Services should only depend on utils and external packages, not on other lib layers or routes.',
			from: { path: '^src/lib/services/' },
			to: { path: '^(src/routes/|src/lib/(components/|stores/|i18n/|assets/))' }
		},
		{
		...
		},
		{
			...
			severity: 'error',
			comment: 'i18n translation data should be a leaf module with no internal dependencies.',
			from: { path: '^src/lib/i18n/' },
			to: { path: '^(src/routes/|src/lib/)', pathNot: ['^src/lib/i18n/'] }
		},
		{
			name: 'utils-not-to-internals',
			severity: 'error',
	...
	options: {
		...
		doNotFollow: {
			...
			path: ['node_modules', '\\.svelte-kit', '^\\$env/', '^\\$app/']
		},
...
// generated: dependency-cruiser@17.4.0 on 2026-05-16T16:04:40.499Z
