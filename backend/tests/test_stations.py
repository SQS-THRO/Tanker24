...
class TestStationList:
    ...
    async def test_list_stations_authenticated_with_stations(
        self, authenticated_client, owned_station
    ):
        response = await authenticated_client.get("/stations/")
    ...
    async def test_list_stations_only_shows_own_stations(
        self, authenticated_client, owned_station, other_user_station
    ):
        response = await authenticated_client.get("/stations/")
...
class TestStationUpdate:
    async def test_update_station_not_found(self, authenticated_client):
        response = await authenticated_client.patch(
            "/stations/99999", json={"name": "Updated Name"}
        )
    ...
    async def test_update_other_user_station(self, authenticated_client, other_user_station):
        response = await authenticated_client.patch(
            f"/stations/{other_user_station.id}", json={"name": "Hacked Name"}
        )
    ...
    async def test_update_station_unauthenticated(self, async_client, owned_station):
        response = await async_client.patch(
            f"/stations/{owned_station.id}", json={"name": "Updated"}
        )
