...
class TestNearbyStationsServiceApi:
    @pytest.mark.asyncio
    async def test_get_nearby_stations_returns_empty_on_api_exception(
    ...
    @pytest.mark.asyncio
    async def test_get_nearby_stations_returns_empty_on_empty_response(
        self, test_db_session
    ):
        async def mock_to_thread(func, *args, **kwargs):
            return []
...
class TestSaveStationsToCache:
    @pytest.mark.asyncio
    async def test_save_stations_to_cache_creates_new_stations(
        self, test_db_session
    ):
        api_stations = [
            GasStation(
        ...
        service = NearbyStationsService(test_db_session)
        await service._save_stations_to_cache(
            api_stations, 52.52, 13.405, 5.0
        )
    ...
    @pytest.mark.asyncio
    async def test_save_stations_to_cache_updates_existing_stations(
        self, test_db_session
    ):
        now = datetime.now(UTC)
