...
class TestInvitationKeyRegistration:
	...
	async def test_user_linked_to_invitation_key(self, test_db_session, client_with_invitation_keys):
		...
		result = await test_db_session.execute(select(User).where(User.email == "newuser@example.com"))
		user = result.scalar_one()
		assert user.invitation_key_id is not None

		result = await test_db_session.execute(select(InvitationKey).where(InvitationKey.key == "a" * 32))
